...
(function (root, factory) {
    ...
    ko.bindingHandlers.selection = {
        ...
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            if (ko.utils.domData.get(element, 'knockout.selection.initialized')) {
                // Make sure the binding is only initialized once.

                // TODO when we upgrade to Knockout 2.2.0 we can avoid
                // this by using peek on the observables we read in
                // cleanup and when initializing single mode
                return;
            }
            ko.utils.domData.set(element, 'knockout.selection.initialized', true);
