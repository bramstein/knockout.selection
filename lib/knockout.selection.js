    ...
    ko.bindingHandlers.selection = {
        modes: {
            single: function (selectionModel) {
                ...
                if (selectionModel.direction === 'vertical' || selectionModel.direction === 'both') {
                ...
                } else if (selectionModel.direction === 'horizontal' || selectionModel.direction === 'both') {
                    matchers.register({ type: 'keydown', which: 37 }, function (event, item) {
                        var previousItem = selectionModel.previousItem(item);
                        selectionModel.selectItem(previousItem);
                        selectionModel.anchor(previousItem);
                    });

                    matchers.register({ type: 'keydown', which: 39 }, function (event, item) {
                        var nextItem = selectionModel.nextItem(item);
                        selectionModel.selectItem(nextItem);
                        selectionModel.anchor(nextItem);
                    });
            ...
            multi: function (selectionModel) {
                ...
                if (selectionModel.direction === 'vertical' || selectionModel.direction === 'both') {
                    ...
                    matchers.register(
                        { type: 'keydown', which: 37, ctrlKey: true, shiftKey: true },
                        { type: 'keydown', which: 37, metaKey: true, shiftKey: true }, function (event, item) {
                        selectionModel.appendSelectionFromAnchor(selectionModel.previousItem(item));
                    });

                    matchers.register(
                        { type: 'keydown', which: 37, ctrlKey: true },
                        { type: 'keydown', which: 37, metaKey: true }, function (event, item) {
                        var prev = selectionModel.previousItem(item);
                        selectionModel.focused(prev);
                        selectionModel.anchor(prev);
                    });

                    matchers.register({ type: 'keydown', which: 37, shiftKey: true }, function (event, item) {
                        selectionModel.replaceSelectionWithRangeFromAnchor(selectionModel.previousItem(item));
                    });
                    ...
                    matchers.register(
                        { type: 'keydown', which: 39, ctrlKey: true, shiftKey: true },
                        { type: 'keydown', which: 39, metaKey: true, shiftKey: true }, function (event, item) {
                        selectionModel.appendSelectionFromAnchor(selectionModel.nextItem(item));
                    });

                    matchers.register(
                        { type: 'keydown', which: 39, ctrlKey: true },
                        { type: 'keydown', which: 39, metaKey: true }, function (event, item) {
                        var next = selectionModel.nextItem(item);
                        selectionModel.focused(next);
                        selectionModel.anchor(next);
                    });

                    matchers.register({ type: 'keydown', which: 39, shiftKey: true }, function (event, item) {
                        selectionModel.replaceSelectionWithRangeFromAnchor(selectionModel.nextItem(item));
                    });
        ...
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            if (bindingValue.selection) {
                ...
                selectionModel.direction = bindingValue.direction || config.direction;
                ...
                selectionModel.direction = config.direction;
            }
