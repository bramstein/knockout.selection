    ...
    ko.bindingHandlers.selection = {
        init: function (element, valueAccessor, allBindingsAccessor, viewModel, bindingContext) {
            ...
            function createMultiModeEventMatchers() {
                ...
                matchers.click.register({ which: 1, ctrlKey: true, shift: true }, function (event, item) {
                    appendSelectionFromAnchor(item);
                });

                matchers.click.register({ which: 1, ctrlKey: true }, function (event, item) {
                    if (isAlreadySelected(item)) {
                ...
                matchers.key.register({ which: 38, ctrlKey: true, shiftKey: true }, function (event, item) {
                    appendSelectionFromAnchor(previousItem(item));
                });

                matchers.key.register({ which: 38, ctrlKey: true }, function (event, item) {
                ...
                matchers.key.register({ which: 40, ctrlKey: true, shiftKey: true }, function (event, item) {
                    appendSelectionFromAnchor(nextItem(item));
                });

                matchers.key.register({ which: 40, ctrlKey: true }, function (event, item) {
